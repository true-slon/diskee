...
public final class SpecificationUtils {
    ...
    public static <X> Specification<X> equalsIgnoreCase(SingularAttribute<X, String> attribute, String value) {
        return (root, query, builder) -> {
            if (value == null) return builder.conjunction();
            return builder.equal(builder.lower(root.get(attribute)), value.toLowerCase());
        };
    }

    public static <X> Specification<X> like(SingularAttribute<X, String> attribute, String value) {
        return (root, query, builder) -> {
            if (value == null) return builder.conjunction();
            return builder.like(
    ...
    public static <X, Y> Specification<X> equalsOrNull(SingularAttribute<X, Y> attribute, Y value) {
        return (value != null) ? equals(attribute, value) : isNull(attribute);
    }
    ...
    public static <X, Y> Specification<X> in(ListAttribute<X, Y> attribute, Collection<Y> value) {
        return (root, query, builder) -> {
            if (value == null) return builder.conjunction();
            return root.join(attribute, JoinType.LEFT).in(value);
        };
    }
    ...
    public static <X> Specification<X> deleted(boolean deleted) {
        return (root, query, criteriaBuilder) -> {
            if (deleted) return criteriaBuilder.conjunction();
            return root.get("deletedAt").isNull();
        };
    }

    public static <X> Specification<X> afterOrEquals(SingularAttribute<X, ZonedDateTime> attribute, ZonedDateTime value) {
        return (root, query, builder) -> {
            if (value == null) return builder.conjunction();
            return builder.greaterThanOrEqualTo(root.get(attribute), value);
        };
    }

    public static <X> Specification<X> beforeOrEquals(SingularAttribute<X, ZonedDateTime> attribute, ZonedDateTime value) {
        return (root, query, builder) -> {
            if (value == null) return builder.conjunction();
            return builder.lessThanOrEqualTo(root.get(attribute), value);
        };
    }

    // JsonNullable заменён на Optional
